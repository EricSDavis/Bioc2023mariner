...
this later. We can use the `DelayedArray::rowSums()`
function to filter out any low-count loops to improve our
power to call differential loops:

```{r}
keep <- which(DelayedArray::rowMeans(counts(pixels)) >= 3)
pixels <- pixels[keep]
```

Then we can create a `DESeqDataSet` and
run `DESeq()`:
